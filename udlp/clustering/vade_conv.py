...
#        self._dec = nn.Linear(decodeLayer[-1], input_dim)
    ...
    def initialize_gmm(self, dataloader):
        ...
        for batch_idx, inputs in enumerate(dataloader):
            #inputs = inputs.view(inputs.size(0), -1).float() # no need to flatten
            if use_cuda:
                inputs = inputs.cuda()
...
#            print(f"self.u_p = {self.u_p}")
    ...
    def decode(self, z):
        ...
        it exists because a big part of our model is pre-trained and _dec and _enc shoe-horns
        it into the data that we're given.
        
        """
        h = self.decoder(z)
        #print(f'decode h.shape {h.shape}')
        x = h
#        x = self._dec(h)
...
#        log_norm = -0.5 * (torch.log(2* math.pi * lambda_tensor3) + (Z - u_tensor3)**2/lambda_tensor3 )
        ...
        if self.debug:
            ...
            print(f"p_c_z.shape = {p_c_z.shape}")
...
#            (1-x)*torch.log(torch.clamp(1-recon_x, min=1e-10)), 1)o
        ...
        if self.debug:
            ...
            print(f"logpzc.shape = {logpzc.shape}")
            print(f"qentropy.shape = {qentropy.shape}")
            print(f"logpc.shape = {logpc.shape}")
            print(f"logqcx.shape = {logqcx.shape}")
    ...
    def forward(self, x):
        ...
        # I flatten it, but now I think this wrong because I lose the batch size
        # we attempt to keep batch dim
        flat_h = h.view(h_shape[0], -1)
        #print(f'flat_h.shape {flat_h.shape}')
        
        # remember that _enc_mu is a linear transform 
        mu = self._enc_mu(flat_h)
        logvar = self._enc_log_sigma(flat_h)
        # i need an inverse to the self._enc ...##############
        z = self.reparameterize(mu, logvar)
        pre_decode = self._pre_decoder(z)
        
        # I'm confused by what happens with batch dimensions...
        # I can't do this here, I think, because it won't be a cuda function...
        ### pre_decode = nn.Linear(self.z_dim, ********* )(z)
        z_reshaped = pre_decode.view(h_shape ) 
        # decode should be strictly conv
        
        return z, self.decode(z_reshaped), mu, logvar
